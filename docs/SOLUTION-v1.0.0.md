...
PostgreSQL (Shared DB)
  ├── Row Level Security (V8 migration)
...
│       └── db/migration/   # Flyway migrations V1→V8 (V8 = RLS)
│
...
├── organizationId: UUID (nullable FK → organizations) ← NULL = platform-level client
...
Seeded clients (DataSeedingConfig `@Order(10)` → clients, `@Order(30)` → metadata):

| clientId | type | org |
|---|---|---|
| `web-client` | WEB_CLIENT | platform |
...
Chain 2 (Order 2): Application endpoints
  ├── /login, /logout, /invite/**, /api/v1/auth/** → permitAll
...
Organization: platform
    └── web-client               (WEB_CLIENT — platform default)
```
...
*Identity Platform v1.0.0 — Self-hosted IdP for B2B Multi-tenant*
